...
class LookupResult:
    ...
    rdap_payload: dict[str, Any] | None = None
    whois_text: str | None = None
    whois_record: Any | None = None
...
class DomainLookup:
    ...
    async def lookup_many(self, domains: list[str]) -> list[LookupResult]:
        unique = list(dict.fromkeys(self._normalize(d) for d in domains))
        tasks = [self._lookup_one(domain) for domain in unique]
        return list(await asyncio.gather(*tasks))

    async def _lookup_one(self, domain: str) -> LookupResult:
        async with self._semaphore:
            registry = self._registry_for_domain(domain)

            rdap = await self._lookup_rdap(domain, registry)
            if rdap.status != AvailabilityStatus.UNKNOWN:
                return rdap

            if registry and registry.whois_host:
                socket_result = await asyncio.to_thread(
                    self._lookup_socket_whois, domain, registry
                )
                if socket_result.status != AvailabilityStatus.UNKNOWN:
                    return socket_result

            return await asyncio.to_thread(self._lookup_whois, domain, registry)
    ...
    def _tld(self, domain: str) -> str:
        ...
        return extracted.suffix.lower() if extracted.suffix else ""
    ...
    async def _lookup_rdap(self, domain: str, registry: TldRegistry | None) -> LookupResult:
        ...
        if registry and registry.rdap_urls:
            override = [_RdapServer(base_url=u.rstrip("/")) for u in registry.rdap_urls]
        ...
        async with httpx.AsyncClient(
            ...
            for server in servers:
                ...
                if response.status_code == 404:
                    ...
                    if UNAVAILABLE_PATTERNS.search(body_text):
                        return LookupResult(
                            ...
                            available=False,
                            method="rdap",
                            message="Not registered but not available for registration",
                        )
                    return LookupResult(
                        domain=domain,
                ...
                if response.status_code == 200:
                    ...
                    registrar, expiry = _parse_rdap_summary(payload)
                    return LookupResult(
                        domain=domain,
    ...
    def _lookup_socket_whois(self, domain: str, registry: TldRegistry) -> LookupResult:
        try:
    ...
    def _lookup_whois(self, domain: str, registry: TldRegistry | None) -> LookupResult:
        try:
...
def _parse_rdap_summary(payload: dict[str, Any]) -> tuple[str | None, str | None]:
    registrar: str | None = None
            ...
            if isinstance(vcard, list) and len(vcard) > 1:
                for row in vcard[1]:
                    if isinstance(row, list) and len(row) >= 4 and row[0] == "fn":
                        registrar = str(row[3])
                        break
            if not registrar and entity.get("handle"):
                registrar = str(entity["handle"])
    return registrar, expiry


def _parse_whois_lookup(
    *,
    domain: str,
    raw_text: str,
    method: str,
    registry: TldRegistry | None,
    record: Any = None,
) -> LookupResult:
    ...
    if record is not None and _is_whois_registered(record, raw_text):
        return LookupResult(
            ...
            expiry_date=_format_date(_record_field(record, "expiration_date")),
            message="Domain is registered",
            whois_text=raw_text,
            whois_record=record,
        )
...
def _is_whois_registered(record: Any, raw_text: str) -> bool:
    ...
    if REGISTERED_PATTERNS.search(raw_text):
        return True
    if _record_field(record, "status"):
        return True
    return False
